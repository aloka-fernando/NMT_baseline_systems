...
tokenizer = MBart50TokenizerFast.from_pretrained("facebook/mbart-large-50", src_lang="en_XX", tgt_lang="ta_IN")

#training data
train_data=[]
with open('data/parallel-27.04.2021-train90KDD.si-en-ta.en') as f1, open('data/parallel-27.04.2021-train90KDD.si-en-ta.ta') as f2:
...
#Validationi data
valid_data=[]
with open('data/parallel-27.04.2021-tu.un.si-en-ta.en') as f1, open('data/parallel-27.04.2021-tu.un.si-en-ta.ta') as f2:
    for src, tgt in zip(f1, f2):
      valid_data.append(
          {
              "translation": {
...
def data_collator(features:list):
  ...
  with tokenizer.as_target_tokenizer():
    label = tokenizer(labels, return_tensors="pt", max_length=115, padding=True, truncation=True).input_ids
...
args = Seq2SeqTrainingArguments(output_dir='mbart50-ft-en-ta-run4',
                        do_train=True,
                        do_eval=True,
                        ...
                        push_to_hub=False,
                        push_to_hub_model_id='mbart50-ft-en-ta-run4')
...
tokenizer.save_pretrained('mbart50-ft-en-ta-run4')
...
mints = int(((elapsedTime%86400)%3600)//60)
